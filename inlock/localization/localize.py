...
import gc
import json
from inlock.localization.device_only import pnp_solver
import viz2d
...
def pairs_from_retrieval(device: torch.device, query_desc: RetrievalFeatures, db_desc : RetrievalFeatures, num_matched:int=5, min_score:int = -1):
    ...
    # Avoid self-matching
    invalid = torch.zeros((query_desc.features.shape[0],db_desc.features.shape[0],), dtype=torch.bool)
    for i, db_name in enumerate(db_desc.db_names):
        invalid[i] = query_desc.db_names == db_name
...
def estimate_pose_retrieval_only(trajectories: Dict[str,ImagePose], matches: List[Tuple[str,str,float]]):
    ...
    for i, (query, ref, score) in enumerate(matches):
        name = ref[ref.find("/") + 1:ref.find(".")].replace("__", "_")
        pose = trajectories[name]
...
def get_trajectory_name(ref: str) -> str:
    name = ref[ref.find("/") + 1:ref.find(".")].replace("__", "_")
    return name
...
def visualise_extracted_features(image, features):
...
def visualise_matches(image0, image1, kpts0, kpts1, m_kpts0, m_kpts1, matches01):
    axes = viz2d.plot_images([image0.numpy(), image1])
...
class LocalizationModule(nn.Module):
    ...
    def extract_local_features(self, image, db_name: str ="tmp") -> LocalFeatures:
        ...
        #visualise_extracted_features(image, pred)
    ...
    def match_features(self, features1: LocalFeatures, features2: LocalFeatures, query_image: ImageDataset,
                       db_image: ImageDataset, retrieval_matches: List[Tuple[str, str, float]], trajectories: Dict[str,ImagePose], intrinsics: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        scores = torch.zeros((len(retrieval_matches),))
        t = torch.zeros((len(retrieval_matches), 3))
        R = torch.zeros((len(retrieval_matches), 3, 3))

        for idx, (name_i, name_j, score) in enumerate(retrieval_matches):
            i = names2idx1[name_i]
            j = names2idx2[name_j]
            ...
            keypoints0 = features1.keypoints[i]
            keypoints1 = features2.keypoints[j]
            ...
            image_path = "../data/HG_navviz/raw_data/images_undistr_center/" + name_j


            matches = match_result["matches"][0]
            print("Matches = ", matches.shape[0], image_path)

            p = features2.p3d[j]

            #ref_image = cv2.imread(image_path)
            #plot_pose_on_map(Path("../data/HG_navviz"), "../data/tmp/fig.png", torch.cat([torch.zeros(p.shape[0], 4,), p], dim=1))
            #visualise_matches(query_image.images[i], ref_image, keypoints0, keypoints1, keypoints0[matches[:,0]], keypoints1[matches[:,1]], rbd(match_result))

            print("==== Matched =====")
            pose = trajectories[get_trajectory_name(name_j)]

            rot_matrix, pos, ratio = pnp_solver.pnp_solve(intrinsics, {
            ...
            print("Camera position ", t[idx])
            print("Retrieval position ", pose.t)
    ...
    def forward(self, query_image: torch.Tensor, intrinsics : torch.Tensor):
        #t3 = time.time()
        ...
        R, t, scores = self.match_features(feature_desc, self.db_feature_desc, query_image_dataset, self.db_image, matches, self.trajectories, intrinsics)

        #R, t, scores = estimate_pose(self.trajectories, kp_matches)

        #t5 = time.time()
        #logger.info(f"Localize : {(t5 - t1) * 1000} ms")
        return torch.cat([R.reshape((-1,9)), t, scores.unsqueeze(1)], dim=1)
...
def compile_model():
    ...
    retrieval_conf = {
        "output": "global-feats-netvlad_600",
        "model": {"name": "netvlad"},
        "preprocessing": {"resize_max": 600},
    }
    ...
    query_image = cv2.imread("../data/tmp/00547-cam3__center.jpg") #00162-cam3__center.jpg")

    size = query_image.shape
    scale = min(feature_conf["preprocessing"]["resize_max"], retrieval_conf["preprocessing"]["resize_max"]) / max(size)
    size_new = (int(size[1] * scale), int(size[0] * scale))
    query_image = cv2.resize(query_image, size_new)
    query_image = torch.from_numpy(query_image)

    intrinsics = torch.Tensor([960, 960, 639.8245614035088, 959.8245614035088])
    print("Max image size", min(feature_conf["preprocessing"]["resize_max"], retrieval_conf["preprocessing"]["resize_max"]))
    print("Resizing intrisics to match scale", scale)
    ...
    final_pose = localizer(query_image, intrinsics)
    t2 = time.time()
    ...
    serialized_output = serialized_model(query_image.cpu(), intrinsics)
